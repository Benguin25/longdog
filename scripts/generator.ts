...
function buildPlan(): SlotSpec[] {
  ...
  for (let n = 1; n <= 100; n++) {
    ...
    if (n <= 4) {
      ...
      if (s.twoDogs) s.tags.push('two-dogs');
      s.spikes = n % 2 === 1;
      s.pits = n % 4 === 2;
      s.reach = n % 3 === 0;
      s.dogLen = 3;
...
function generateSlot(spec: SlotSpec, seed: number): Chosen {
  ...
  // Tier 1: strict par band. Tier 2: widened band. Tier 3: widened + greedy waived.
  for (let tier = 1; tier <= 3; tier++) {
    const widen = tier === 1 ? 0 : 4;
    const parMin = Math.max(3, spec.parMin - widen);
    const parMax = spec.parMax + widen;
    const specForTier: SlotSpec = tier === 3 ? { ...spec, allowGreedy: true } : spec;
    const maxAttempts = tier === 1 ? 900 : 450;
...
function runCli(): void {
  ...
  for (const spec of plan) {
    const t = Date.now();
    const c = generateSlot(spec, seed);
